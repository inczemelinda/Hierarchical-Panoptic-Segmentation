...
from .modeling.hierarchical_criterion import SetCriterion
...
class MaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            plant_mask_cls_results = outputs["plant_pred_logits"]
            plant_mask_pred_results = outputs["plant_pred_masks"]
            leaf_mask_cls_results = outputs["leaf_pred_logits"]
            leaf_mask_pred_results = outputs["leaf_pred_masks"]
            # upsample masks
            for key in ["plant_pred_masks", "leaf_pred_masks"]:
            ...
            processed_results = []
            for i in range(len(plant_mask_cls_results)):
                ...
                for level in ["plant", "leaf"]:
                    if self.sem_seg_postprocess_before_inference:
                        mask_pred_result = retry_if_cuda_oom(sem_seg_postprocess)(
                            outputs[f"{level}_pred_masks"][i], image_size, height, width
                        )
                        mask_cls_result = outputs[f"{level}_pred_logits"][i].to(mask_pred_result)

                    else:
                        mask_pred_result = outputs[f"{level}_pred_masks"][i]
                        mask_cls_result = outputs[f"{level}_pred_logits"][i]
                    ...
                    if self.panoptic_on:
                        if level == "leaf":
                            panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, mask_pred_result, 2, "leaf")
                            processed_results[-1][f"{level}_panoptic_seg"] = panoptic_r
                        else:
                            panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, mask_pred_result, 3, "plant")
                            processed_results[-1][f"{level}_panoptic_seg"] = panoptic_r
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_image in targets:
            ...
            for level in ["plant", "leaf"]:
                ...
                new_target[f"{level}_masks"] = padded_masks
    ...
    def panoptic_inference(self, mask_cls, mask_pred, num_classes, level):
        ...
        if cur_masks.shape[0] == 0:
            ...
            for k in range(cur_classes.shape[0]):
                ...
                if level == "plant":
                    isthing = pred_class in [1, 2]
                else:
                    isthing = pred_class in [1]
