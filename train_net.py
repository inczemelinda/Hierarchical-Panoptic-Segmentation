...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        # Optionally use RepeatFactorTrainingSampler to oversample images with rare classes (weeds).
        # Reads per-image factors from repeat_factors.json (generated by compute_repeat_factors.py).
        sampler = None
        if cfg.DATALOADER.SAMPLER_TRAIN == "RepeatFactorTrainingSampler":
            import json as _json
            from detectron2.data.samplers import RepeatFactorTrainingSampler

            rf_file = "repeat_factors.json"
            if not os.path.exists(rf_file):
                raise FileNotFoundError(
                    f"{rf_file} not found. Run: python compute_repeat_factors.py"
                )
            with open(rf_file) as f:
                rf_dict = _json.load(f)
            # Build per-sample repeat factors in the SAME order as the dataset
            filenames = dataset.filenames  # PhenoBench keeps filenames sorted
            repeat_factors = torch.tensor(
                [rf_dict.get(fn, 1.0) for fn in filenames], dtype=torch.float32
            )
            sampler = RepeatFactorTrainingSampler(repeat_factors)
            # Use detectron2 logger (captured in log.txt); also print to stdout for visibility
            msg = (f"[RepeatFactorTrainingSampler] Loaded {len(repeat_factors)} repeat factors "
                   f"(mean={repeat_factors.mean():.2f}, max={repeat_factors.max():.2f})")
            logging.getLogger("detectron2").info(msg)
            print(msg)

        return build_detection_train_loader(
            dataset=dataset, mapper=mapper, sampler=sampler,
            aspect_ratio_grouping=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
